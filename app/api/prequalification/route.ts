...
export async function POST(request:Request) {
  try {
    ...
    const {data,error}=await resend.emails.send({
      ...
      html:`<div style="font-family:Arial,sans-serif;color:#111;max-width:620px"><p style="color:#ec007c;font-size:12px;letter-spacing:.12em">NUEVA PRECLASIFICACIÓN ORCA</p><h1>${escapeHtml(answers.company)}</h1><p><strong>Razón social:</strong> ${escapeHtml(answers.legalName)}</p><p><strong>Ubicación:</strong> ${escapeHtml(answers.location)}</p><p><strong>Compatibilidad:</strong> ${result.compatibility} (${result.total}/100)</p><p><strong>Clasificación:</strong> ${result.companyClass}</p><p><strong>Contacto:</strong> ${escapeHtml(answers.contact)} · ${escapeHtml(answers.email)}</p><p><strong>WhatsApp:</strong> ${escapeHtml(answers.whatsapp)}</p>${research?`<p><strong>Investigación pública:</strong> incluida en el PDF con ${research.sources.length} fuente(s).</p>`:`<p><strong>Investigación pública:</strong> no disponible; se adjunta el reporte de preclasificación.</p>`}<p>El reporte completo está adjunto en PDF.</p></div>`,
