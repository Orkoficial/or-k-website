...
const initialAnswers: Answers = {
  company:"", contact:"", email:"", whatsapp:"", source:"", years:"", website:"", channels:[],
  ...
  return <div className="contactPanel prequalPanel" role="dialog" aria-modal="true" aria-label="Preclasificación empresarial ORCA">
    ...
    {!complete?<>
      ...
      <div className="prequalBody">
        ...
        {step===0&&<><h3>Empecemos por<br/>conocerte.</h3><div className="prequalFields"><label>Nombre de la empresa<input value={answers.company} onChange={e=>set("company",e.target.value)} autoFocus/></label><label>Tu nombre<input value={answers.contact} onChange={e=>set("contact",e.target.value)}/></label><label>Correo corporativo<input type="email" value={answers.email} onChange={e=>set("email",e.target.value)}/></label><label>Número de WhatsApp<input type="tel" inputMode="tel" autoComplete="tel" value={answers.whatsapp} onChange={e=>set("whatsapp",e.target.value)} placeholder="+57 300 123 4567" aria-describedby="whatsapp-help"/><small id="whatsapp-help">Incluye el indicativo del país</small></label><label>Website <small>(opcional)</small><input type="url" value={answers.website} onChange={e=>set("website",e.target.value)} placeholder="https://"/></label></div><Question number="01" title="¿Cómo conociste ORCA?"><ChoiceGroup name="Origen" values={options.source} selected={answers.source} onSelect={value=>set("source",value)}/></Question><Question number="02" title="¿Cuántos años lleva operando?"><ChoiceGroup name="Años operando" values={options.years} selected={answers.years} onSelect={value=>set("years",value)}/></Question><Question number="03" title="¿En qué canales tiene presencia?" hint="Puedes elegir varias"><ChoiceGroup multiple name="Canales" values={options.channels} selected={answers.channels} onSelect={value=>toggle("channels",value)}/></Question></>}
