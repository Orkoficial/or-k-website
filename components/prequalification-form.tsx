...
function Result({answers,result,delivery,onRestart}:{answers:Answers;result:ReturnType<typeof scorePrequalification>;delivery:"idle"|"sending"|"sent"|"error";onRestart:()=>void}) {
  ...
  return <div className="prequalResult">
    ...
    <p className={`prequalDelivery is-${delivery}`}>{delivery==="sending"?"Investigando la empresa y generando el reporte PDF…":delivery==="sent"?"Reporte enriquecido enviado correctamente a ORCA.":delivery==="error"?"Tu resultado se generó correctamente, pero el PDF no pudo enviarse. Intenta nuevamente.":""}</p>
