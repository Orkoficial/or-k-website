...
const initialAnswers: Answers = {
  company:"", legalName:"", taxId:"", location:"", linkedin:"", contact:"", email:"", whatsapp:"", source:"", years:"", website:"", channels:[],
...
export default function PrequalificationForm({onClose}:{onClose:()=>void}) {
  ...
  const validWhatsapp=/^\+?[\d\s()-]{7,20}$/.test(answers.whatsapp);
  const valid = step===0?Boolean(answers.company&&answers.legalName&&answers.location&&answers.contact&&answers.email&&validWhatsapp&&answers.source&&answers.years):step===1?Boolean(answers.revenue&&answers.employees&&answers.payroll):step===2?Boolean(answers.tools.length&&answers.challenge.length):Boolean(answers.urgency&&answers.leadership&&answers.investment);
  ...
  return <div className="contactPanel prequalPanel" role="dialog" aria-modal="true" aria-label="Preclasificación empresarial OR-K">
    <button className="contactClose" type="button" onClick={onClose} aria-label="Cerrar formulario">×</button>
    {!complete?<>
      <header className="prequalHeader"><span>OR-K / Preclasificación empresarial</span><strong>{String(step+1).padStart(2,"0")} / {String(steps.length).padStart(2,"0")}</strong></header>
      ...
      <div className="prequalBody">
        ...
        {step===0&&<><h3>Empecemos por<br/>conocerte.</h3><div className="prequalFields"><label>Nombre comercial<input value={answers.company} onChange={e=>set("company",e.target.value)} autoFocus/></label><label>Razón social<input value={answers.legalName} onChange={e=>set("legalName",e.target.value)} placeholder="Nombre legal completo"/></label><label>País y ciudad<input value={answers.location} onChange={e=>set("location",e.target.value)} placeholder="Colombia, Bogotá"/></label><label>NIT <small>(opcional)</small><input value={answers.taxId} onChange={e=>set("taxId",e.target.value)} placeholder="900.000.000-0"/></label><label>Tu nombre<input value={answers.contact} onChange={e=>set("contact",e.target.value)}/></label><label>Correo corporativo<input type="email" value={answers.email} onChange={e=>set("email",e.target.value)}/></label><label>Número de WhatsApp<input type="tel" inputMode="tel" autoComplete="tel" value={answers.whatsapp} onChange={e=>set("whatsapp",e.target.value)} placeholder="+57 300 123 4567" aria-describedby="whatsapp-help"/><small id="whatsapp-help">Incluye el indicativo del país</small></label><label>Website <small>(opcional)</small><input type="url" value={answers.website} onChange={e=>set("website",e.target.value)} placeholder="https://"/></label><label>LinkedIn de la empresa <small>(opcional)</small><input type="url" value={answers.linkedin} onChange={e=>set("linkedin",e.target.value)} placeholder="https://linkedin.com/company/..."/></label></div><Question number="01" title="¿Cómo conociste OR-K?"><ChoiceGroup name="Origen" values={options.source} selected={answers.source} onSelect={value=>set("source",value)}/></Question><Question number="02" title="¿Cuántos años lleva operando?"><ChoiceGroup name="Años operando" values={options.years} selected={answers.years} onSelect={value=>set("years",value)}/></Question><Question number="03" title="¿En qué canales tiene presencia?" hint="Puedes elegir varias"><ChoiceGroup multiple name="Canales" values={options.channels} selected={answers.channels} onSelect={value=>toggle("channels",value)}/></Question></>}
...
function Result({answers,result,delivery,onRestart}:{answers:Answers;result:ReturnType<typeof scorePrequalification>;delivery:"idle"|"sending"|"sent"|"error";onRestart:()=>void}) {
  ...
  return <div className="prequalResult">
    ...
    <p className="prequalResultLabel">Compatibilidad OR-K</p>
    ...
    <p className={`prequalDelivery is-${delivery}`}>{delivery==="sending"?"Investigando la empresa y generando el reporte PDF…":delivery==="sent"?"Reporte enriquecido enviado correctamente a OR-K.":delivery==="error"?"Tu resultado se generó correctamente, pero el PDF no pudo enviarse. Intenta nuevamente.":""}</p>
    <div className="prequalResultActions"><a href={`mailto:hola@or-k.co?subject=${encodeURIComponent(`Compatibilidad OR-K · ${answers.company}`)}&body=${encodeURIComponent(`Hola OR-K, completé la preclasificación.\n\nEmpresa: ${answers.company}\nContacto: ${answers.contact}\nWhatsApp: ${answers.whatsapp}\nResultado: ${result.compatibility} (${result.total}/100)\nClasificación: ${result.companyClass}`)}`}>{isHigh?"Agendar conversación":"Compartir con OR-K"} <b>↗</b></a><button type="button" onClick={onRestart}>Volver a empezar</button></div>
  </div>;
}
